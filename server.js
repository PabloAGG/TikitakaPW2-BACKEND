...
pool.connect((err, client, release) => {
  ...
  client.query('SELECT NOW()', (err, result) => {
    ...
    console.log(`✅ ¡Conexión a ${dbInfo.type} verificada exitosamente! Hora de la DB:`, timeResult);
  });
});
...
// Endpoint inteligente para productos recomendados
app.get('/api/productos/recomendados/:id', async (req, res) => {
    const { id } = req.params;
    const { limit = 6 } = req.query;
    
    try {
        console.log(`Obteniendo recomendaciones inteligentes para producto ${id}`);
        
        // Primero obtenemos la información del producto base
        const productoQuery = `
            SELECT p.*, s."Nombre" AS seleccionNombre
            FROM producto AS p
            LEFT JOIN selecciones AS s ON p.seleccion = s."idSelec"
        ...
        const { rows: productoRows } = await executeQuery(productoQuery, [id]);
        
        if (productoRows.length === 0) {
            return res.status(404).json({ error: 'Producto no encontrado' });
        }
        
        const productoBase = productoRows[0];
        
        // Query para recomendaciones con múltiples criterios y pesos
        const recomendacionesQuery = `
            WITH recomendaciones_ponderadas AS (
                SELECT DISTINCT ON (p."idProduct")
                    p.*,
                    s."Nombre" AS seleccionNombre,
                    m.url AS img,
                    ...
                    CASE 
                        WHEN p.seleccion = $2 THEN 10  -- Misma selección: peso máximo
                        WHEN p.genero = $3 THEN 7      -- Mismo género: peso alto
                        WHEN p.nombre ILIKE '%' || $4 || '%' THEN 5  -- Nombre similar: peso medio
                        WHEN p.top = true THEN 3       -- Producto destacado: peso bajo
                        ELSE 1                         -- Otros: peso mínimo
                    END as peso_relevancia,
                    -- Puntuación adicional por popularidad (basada en estrellas)
                    COALESCE(
                        (SELECT AVG(valoracion) FROM estrellas WHERE producto = p."idProduct"), 0
                ...
                WHERE p.activo = true 
                    AND p."idProduct" != $1
                ORDER BY p."idProduct", m.idmulti ASC
            )
            SELECT *
            FROM recomendaciones_ponderadas
            WHERE peso_relevancia >= 3  -- Solo productos con relevancia mínima
            ORDER BY 
                peso_relevancia DESC,
                promedio_estrellas DESC,
                "idProduct" DESC
            LIMIT $5
        `;
        
        const { rows } = await executeQuery(recomendacionesQuery, [
            id,                           // $1: ID del producto a excluir
            productoBase.seleccion,       // $2: Selección del producto base
            productoBase.genero,          // $3: Género del producto base  
            productoBase.nombre.split(' ')[0], // $4: Primera palabra del nombre
            parseInt(limit)               // $5: Límite de resultados
        ]);
        
        // Si no hay suficientes recomendaciones con alta relevancia, 
        // agregar algunos productos aleatorios populares
        if (rows.length < parseInt(limit)) {
            const productosAdicionales = parseInt(limit) - rows.length;
            ...
            const fillQuery = `
                ...
                WHERE p.activo = true 
                    AND p."idProduct" != $1
                    AND p."idProduct" NOT IN (${rows.map((_, i) => `$${i + 3}`).join(', ')})
                ORDER BY p."idProduct", m.idmulti ASC, RANDOM()
                LIMIT $2
            `;
            ...
            const fillParams = [
                ...
                ...rows.map(row => row.idProduct)
            ];
            
            const { rows: fillRows } = await executeQuery(fillQuery, fillParams);
            rows.push(...fillRows);
        }
